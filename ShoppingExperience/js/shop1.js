...
function addProducts(individualCard) {
    ...
    return `
        <div class="card" id="cardNumber${individualCard.id}">
            ...
            <div class="cardText">
                ...
                <p>&dollar; ${individualCard.price.toFixed(2)}</p>
                <button class="cartButton" id="${individualCard.id}" type="button">Add to Cart</button>
            </div>
        </div>`;

} 
 // Iterate over the products array and append each product card to the cardContainer
products.forEach((individualCard) => {
    const cardHTML = addProducts(individualCard);
